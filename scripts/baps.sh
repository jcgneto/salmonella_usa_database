...
hb_results <- hierBAPS(snp_matrix, max.depth = 1, n.pops = 50, n.extra.rounds = Inf, quiet = TRUE)
...
hb_results_prob <- hierBAPS(snp_matrix, max.depth = 1, n.pops = 50, n.extra.rounds = Inf, quiet = TRUE, assignment.probs = TRUE)
write.csv(hb_results_prob$cluster.assignment.prob[[1]], file = file.path("/work/benson/netogomes/pop_genomics/salmonella_project_1/data_raw_files/paper_1_analysis/prokka_human_bovine/prokka_output/roary/baps", "hierbaps_probabilities.csv")
